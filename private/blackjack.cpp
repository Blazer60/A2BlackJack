...
void playerTurn(hand &player, hand &dealer, deck &cards, int credits, int bet) {
    ...
    for (;;) {
        ...
        /* Ask if the user want to draw another card? */
        if (!dealCard()) {
            return;
        }
...
void dealerTurn(hand &dealer, deck &cards) {
    calculateTotalScore(dealer);  // in case the hole card has not been counted
...
void displayBetInfo(int &credits, int &bet) {
    /* Get the length of both numbers and subtract it from the spacing */
